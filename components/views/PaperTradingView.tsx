...
export const PaperTradingView: React.FC<PaperTradingViewProps> = ({ portfolio, marketSnapshot, isEmergencyKillSwitchActive = false, onOrderExecuted, onRiskVerdictGenerated }) => {
  const [symbol, setSymbol] = useState(marketSnapshot.symbol);
  ...
  const handleSubmitOrder = () => {
    if (isEmergencyKillSwitchActive) {
      setExecutionLog(prev => [`[${new Date().toLocaleTimeString()}] REJECTED: Emergency kill switch is active.`, ...prev]);
      return;
    }
    ...
    const proposedOrder: OrderRequest = {
      id: newOrderId, orderId: newOrderId, clientOrderId: `CLI-${Date.now()}`, symbol, side, type: orderType, quantity,
      limitPrice: orderType === 'LIMIT' ? limitPrice : marketSnapshot.lastPrice, stopLossPrice, takeProfitPrice,
      executionMode: 'PAPER', timestamp: Date.now(), aiDecisionId: 'MANUAL_PAPER_DESK',
    };

    const verdict = evaluateRiskGates(proposedOrder, portfolio, marketSnapshot, { isEmergencyKillSwitchActive });
    onRiskVerdictGenerated(verdict);
    if (!verdict.isApproved) {
      setExecutionLog(prev => [`[${new Date().toLocaleTimeString()}] REJECTED by Risk Engine: ${verdict.rejectionReasons.join('; ')}`, ...prev]);
    ...
    setTimeout(() => {
      ...
      if (execResult.status === 'FILLED' && execResult.fill) {
        setExecutionLog(prev => [`[${new Date().toLocaleTimeString()}] FILLED: ${side} ${quantity} ${symbol} @ $${execResult.fill!.price.toFixed(2)} (Slippage: ${execResult.fill!.slippageIncurredBps} bps, Fees: $${execResult.totalCharges.toFixed(2)}) - Token: ${verdict.auditableRiskToken.slice(0, 10)}...`, ...prev]);
        onOrderExecuted(execResult.updatedPortfolio, proposedOrder);
      } else {
        setExecutionLog(prev => [`[${new Date().toLocaleTimeString()}] EXECUTION FAILED: ${execResult.rejectionReason || 'Unknown paper execution rejection'}`, ...prev]);
      }
      setIsSubmitting(false);
    }, 350);
  };
  ...
  return (
    <div className="space-y-6">
      <div className="bg-slate-900 border border-slate-800 rounded-lg p-4 flex flex-wrap justify-between items-center gap-4">
        <div><div className="flex items-center space-x-2"><ClipboardList className="w-5 h-5 text-amber-400" /><h2 className="text-sm font-semibold text-slate-100">Paper Trading Execution Console</h2></div><p className="text-xs text-slate-400 mt-1">Sandbox-only execution with modeled spread, slippage, brokerage and taxes.</p></div>
        <div className="flex items-center space-x-2 font-mono text-xs px-2.5 py-1 bg-amber-950/40 border border-amber-500/30 rounded text-amber-300"><ShieldAlert className="w-4 h-4 text-amber-400" /><span>{isEmergencyKillSwitchActive ? 'KILL SWITCH ACTIVE — ORDERS BLOCKED' : 'SIMULATED SANDBOX (REAL MONEY LOCKED)'}</span></div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="bg-slate-900 border border-slate-800 rounded-lg p-5 space-y-4 lg:col-span-2">
          <div className="flex justify-between items-center border-b border-slate-800 pb-3"><h3 className="text-xs font-mono font-bold text-slate-200">ORDER TICKET SPECIFICATION</h3><span className="text-[11px] font-mono text-slate-400">Target Asset: {symbol}</span></div>
          ...
          <button type="button" onClick={handleSubmitOrder} disabled={isSubmitting || isEmergencyKillSwitchActive} className={`w-full py-2.5 rounded font-bold font-mono text-xs flex items-center justify-center space-x-2 ${isEmergencyKillSwitchActive ? 'bg-slate-700' : side === 'BUY' ? 'bg-emerald-600 hover:bg-emerald-700' : 'bg-rose-600 hover:bg-rose-700'} text-white disabled:opacity-50`}>{isSubmitting ? <><RefreshCw className="w-4 h-4 animate-spin" /><span>Evaluating Risk Gates...</span></> : <><ArrowRight className="w-4 h-4" /><span>{isEmergencyKillSwitchActive ? 'ORDERS BLOCKED BY KILL SWITCH' : `SUBMIT ${side} ORDER TO RISK ENGINE`}</span></>}</button>
        </div>
