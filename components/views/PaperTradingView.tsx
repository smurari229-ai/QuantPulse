...
export const PaperTradingView: React.FC<PaperTradingViewProps> = ({ portfolio, marketSnapshot, onOrderExecuted, onRiskVerdictGenerated }) => {
  ...
  return (
    <div className="space-y-6">
      <div className="bg-slate-900 border border-slate-800 rounded-lg p-4 flex flex-wrap justify-between items-center gap-4">
        ...
        <div className="flex items-center space-x-2 font-mono text-xs px-2.5 py-1 bg-amber-950/40 border border-amber-500/30 rounded text-amber-300"><ShieldAlert className="w-4 h-4 text-amber-400" /><span>SIMULATED SANDBOX (REAL MONEY LOCKED)</span></div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="bg-slate-900 border border-slate-800 rounded-lg p-5 space-y-4 lg:col-span-2">
          ...
          <button type="button" onClick={handleSubmitOrder} disabled={isSubmitting} className={`w-full py-2.5 rounded font-bold font-mono text-xs flex items-center justify-center space-x-2 ${side === 'BUY' ? 'bg-emerald-600 hover:bg-emerald-700' : 'bg-rose-600 hover:bg-rose-700'} text-white disabled:opacity-50`}>{isSubmitting ? <><RefreshCw className="w-4 h-4 animate-spin" /><span>Evaluating Risk Gates...</span></> : <><ArrowRight className="w-4 h-4" /><span>SUBMIT {side} ORDER TO RISK ENGINE</span></>}</button>
        </div>
