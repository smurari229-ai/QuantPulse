...
export const FailureSimulatorView: React.FC<FailureSimulatorViewProps> = ({ riskConfig, portfolio, marketSnapshot, onUpdateSnapshot, onUpdateKillSwitch, onRiskVerdictGenerated }) => {
  ...
  const restore = () => { onUpdateSnapshot({ ...marketSnapshot, timestamp: Date.now(), dataQuality: { ...marketSnapshot.dataQuality, latencyMs: 38, isStale: false } }); setActiveScenario(null); log('RESTORED: simulated market feed marked fresh at 38ms. Kill-switch state is unchanged and still requires its normal reset flow.'); };
  const scenarios = [
