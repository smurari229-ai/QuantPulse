...
export const AuditLogsView: React.FC = () => {
  ...
  const handleRefresh = () => {
    setRecords(GlobalAuditLedger.getAllRecords());
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-lg p-4 flex flex-wrap justify-between items-center gap-4">
        <div>
          <div className="flex items-center space-x-2">
            ...
            <h2 className="text-sm font-semibold text-slate-100">Cryptographically Chained Tamper-Evident Audit Ledger</h2>
          </div>
          <p className="text-xs text-slate-400 mt-1">
            Rule 8 & 21 Compliance: Strict SHA-256 hash chaining links every event sequentially. All credentials and secrets are scrubbed.
          </p>
        </div>
        ...
        <div className="flex items-center space-x-3">
          <button
            ...
            className="p-2 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded text-xs transition-colors"
            title="Refresh logs"
          >
          ...
          <button
            ...
            className="flex items-center space-x-1.5 px-3 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded text-xs font-mono font-semibold transition-colors"
          >
      ...
      {integrityStatus && (
        <div className={`p-4 rounded-lg border font-mono text-xs flex items-center space-x-3 ${
          ...
          <div>
            <span className="font-bold block">
              {integrityStatus.isValid
                ? `LEDGER INTEGRITY VERIFIED: All ${integrityStatus.checkedCount} cryptographic blocks match previous hash pointers.`
                : 'INTEGRITY BREACH DETECTED: Hash chain broken or record modified out of band!'}
            </span>
            <span className="text-[11px] text-slate-400">
              Deterministic verification guarantees no silent log mutation, retroactive deletion, or state tampering.
            </span>
          </div>
        </div>
      )}
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-lg p-3.5 flex flex-wrap justify-between items-center gap-3 text-xs font-mono">
        <div className="flex items-center space-x-2 flex-1 max-w-sm">
          ...
          <input
            ...
            className="w-full bg-slate-950 border border-slate-800 rounded px-2.5 py-1 text-slate-200 placeholder-slate-600 focus:outline-none focus:border-blue-500 text-xs"
          />
        </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-lg overflow-hidden font-mono text-xs">
        <div className="px-4 py-2.5 bg-slate-950/40 border-b border-slate-800 flex justify-between items-center text-[11px] text-slate-400">
          ...
          <span className="flex items-center space-x-1 text-emerald-400">
            ...
            <span>Zero-Secret Sanitization Protocol: ACTIVE</span>
          </span>
        </div>
          ...
          {filteredRecords.map((record) => (
            <div key={record.sequenceNumber} className="p-4 hover:bg-slate-800/30 transition-colors space-y-2">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div className="flex items-center space-x-2">
                  <span className="px-2 py-0.5 bg-slate-800 text-slate-400 rounded font-bold text-[10px]">
                    #{record.sequenceNumber}
                  </span>
                  <span className="px-2 py-0.5 bg-blue-950/60 border border-blue-500/40 text-blue-300 rounded font-bold text-[10px]">
                    {record.sourceModule}
                  </span>
                  <span className="font-bold text-slate-100">{record.eventType}</span>
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-[10px] text-slate-400 bg-slate-950/60 p-2 rounded border border-slate-800/60">
                <div className="truncate">
                  ...
                  <span className="text-slate-300 font-mono">{record.previousHash}</span>
                </div>
                <div className="truncate">
                  <span className="text-slate-500">BLOCK HASH: </span>
                  <span className="text-emerald-400 font-mono">{record.recordHash}</span>
                </div>
              </div>

              {/* Payload details */}
              <div className="bg-slate-950/40 p-2.5 rounded border border-slate-800/40 text-[11px] text-slate-300 overflow-x-auto">
