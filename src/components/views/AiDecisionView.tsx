...
export const AiDecisionView: React.FC<AiDecisionViewProps> = ({ decision, indicators, currentPrice, symbol, marketSnapshot, onRefreshDecision }) => {
  ...
  const getSignalBadge = (sig: AIDecisionOutput['signal']) => {
    switch (sig) {
      case 'BUY': return <span className="px-3 py-1 bg-emerald-950/80 border border-emerald-500 text-emerald-400 font-mono font-bold text-sm rounded">BUY SIGNAL</span>;
      case 'SELL': return <span className="px-3 py-1 bg-rose-950/80 border border-rose-500 text-rose-400 font-mono font-bold text-sm rounded">SELL SIGNAL</span>;
      case 'HOLD': return <span className="px-3 py-1 bg-blue-950/80 border border-blue-500 text-blue-400 font-mono font-bold text-sm rounded">HOLD POSITION</span>;
      default: return <span className="px-3 py-1 bg-amber-950/80 border border-amber-500 text-amber-400 font-mono font-bold text-sm rounded">NO TRADE / WAIT</span>;
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="bg-amber-950/30 border border-amber-500/40 rounded-lg p-3.5 flex items-start space-x-3 text-xs text-amber-200/90 font-mono">
        ...
        <div><span className="font-bold text-amber-300 block mb-0.5">RULE 1 & 6 COMPLIANCE MANDATE:</span><span>{decision.confidenceCalibrationNote} The AI Decision Engine produces recommendations strictly subordinate to the deterministic Pre-Trade Risk Engine. The AI is explicitly permitted to return NO_TRADE / WAIT.</span></div>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-lg overflow-hidden">
        ...
        <div className="p-5 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4"><div className="p-4 bg-slate-950/40 rounded-lg border border-slate-800 space-y-2"><div className="flex items-center space-x-1.5 text-xs font-mono font-bold text-rose-400"><AlertTriangle className="w-4 h-4" /><span>IDENTIFIED RISK FLAGS ({decision.risk_flags.length})</span></div>{decision.risk_flags.length === 0 ? <p className="text-xs text-slate-400 font-mono">Zero anomalous risk flags detected in market condition vector.</p> : <ul className="space-y-1.5 text-xs font-mono">{decision.risk_flags.map((flag, idx) => <li key={idx} className="flex items-center space-x-2 text-rose-300/90 bg-rose-950/30 px-2 py-1 rounded border border-rose-900/40"><span className="w-1.5 h-1.5 rounded-full bg-rose-400" /><span>{flag}</span></li>)}</ul>}</div><div className="p-4 bg-slate-950/40 rounded-lg border border-slate-800 space-y-2"><div className="flex items-center space-x-1.5 text-xs font-mono font-bold text-emerald-400"><Shield className="w-4 h-4" /><span>MANDATORY CHECKS REQUIRED FOR RISK ENGINE</span></div><ul className="space-y-1.5 text-xs font-mono">{decision.required_checks.map((chk, idx) => <li key={idx} className="flex items-center space-x-2 text-emerald-300/90 bg-emerald-950/30 px-2 py-1 rounded border border-emerald-900/40"><CheckCircle2 className="w-3.5 h-3.5 text-emerald-400 shrink-0" /><span>{chk}</span></li>)}</ul></div></div>
          {showRawJson && <div className="space-y-2"><span className="text-xs font-mono text-slate-400 uppercase font-semibold">Strict JSON Schema Contract:</span><pre className="p-4 bg-slate-950 text-emerald-400 font-mono text-xs rounded border border-slate-800 overflow-x-auto">{JSON.stringify(decision, null, 2)}</pre></div>}
        </div>
      </div>
    </div>
  );
