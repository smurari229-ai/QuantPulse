...
export const SUPPORTED_INSTRUMENTS: InstrumentMetadata[] = [
  {
    symbol: 'NIFTY50', name: 'Nifty 50 Index Basket', assetClass: 'INDEX', exchange: 'NSE', currency: 'INR', lotSize: 25, tickSize: 0.05, marginRequirement: 0.20, maxLeverage: 5,
    tradingHours: { open: '09:15', close: '15:30', timezone: 'Asia/Kolkata' },
  },
  {
    symbol: 'RELIANCE', name: 'Reliance Industries Ltd.', assetClass: 'EQUITY', exchange: 'NSE', currency: 'INR', lotSize: 1, tickSize: 0.05, marginRequirement: 0.25, maxLeverage: 4,
    tradingHours: { open: '09:15', close: '15:30', timezone: 'Asia/Kolkata' },
  },
  {
    symbol: 'TCS', name: 'Tata Consultancy Services', assetClass: 'EQUITY', exchange: 'NSE', currency: 'INR', lotSize: 1, tickSize: 0.05, marginRequirement: 0.25, maxLeverage: 4,
    tradingHours: { open: '09:15', close: '15:30', timezone: 'Asia/Kolkata' },
  },
  {
    symbol: 'SPY', name: 'SPDR S&P 500 ETF Trust', assetClass: 'EQUITY', exchange: 'NYSE', currency: 'USD', lotSize: 1, tickSize: 0.01, marginRequirement: 0.25, maxLeverage: 4,
    tradingHours: { open: '09:30', close: '16:00', timezone: 'America/New_York' },
  },
  {
    symbol: 'BTC-USD', name: 'Bitcoin / US Dollar', assetClass: 'CRYPTO', exchange: 'COINBASE', currency: 'USD', lotSize: 0.001, tickSize: 0.1, marginRequirement: 0.50, maxLeverage: 2,
...
export function generateRealisticHistoricalOHLCV(symbol: string, daysCount = 120, basePrice?: number, volatility = 0.015): OHLCV[] {
  ...
  for (let i = 0; i < daysCount; i++) {
    ...
    if (i % 25 === 0) currentDrift = Math.sin(i / 10) * 0.001;
...
export function validateMarketDataSeries(bars: OHLCV[]): MarketValidationResult {
  ...
  if (!Array.isArray(bars) || bars.length === 0) {
    return { isValid: false, errors: ['Market data series is empty or undefined'], warnings: [], anomaliesDetected: { negativePrice: true, highLowInversion: false, zeroVolumeSpike: false, priceGapExceedsThreshold: false, staleTimestamp: true } };
  }

  for (let i = 0; i < bars.length; i++) {
    const bar = bars[i];
    const valuesAreFinite = [bar.timestamp, bar.open, bar.high, bar.low, bar.close, bar.volume].every(Number.isFinite);
    if (!valuesAreFinite) {
    ...
    if (i > 0 && Number.isFinite(bars[i - 1].close) && bars[i - 1].close > 0) {
      ...
      if (jumpPct > 0.20) { priceGapExceedsThreshold = true; warnings.push(`Bar #${i}: Extreme price jump of ${(jumpPct * 100).toFixed(1)}% between bars.`); }
      if (bar.timestamp <= prevBar.timestamp) { staleTimestamp = true; errors.push(`Bar #${i}: Non-chronological or duplicate timestamp (${bar.timestamp} <= ${prevBar.timestamp}).`); }
    }
  }
  return { isValid: errors.length === 0, errors, warnings, anomaliesDetected: { negativePrice, highLowInversion, zeroVolumeSpike, priceGapExceedsThreshold, staleTimestamp } };
}
...
export function getLiveSnapshot(symbol: string, lastClose: number, options?: { isStale?: boolean; latencyMs?: number; injectAnomaly?: boolean }): MarketDataSnapshot {
  ...
  return {
    ...
    bid, ask, bidSize: 450, askSize: 520, volume24h: 1420500, high24h: lastClose * 1.018, low24h: lastClose * 0.985, change24h: 0.85,
    dataQuality: { isStale, latencyMs: latency, missingCandlesCount: 0, isValidated: !options?.injectAnomaly && !isStale, provider: 'NSE_TICK_FEED_SIMULATOR' },
  };
}
